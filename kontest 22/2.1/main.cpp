...
public:
    ...
    bool trySet(int color, std::vector<std::vector<int>>& g, std::vector<Vertex>& vertex) {
        marked = true;
        if (color == color1 /*|| color == 0*/) {
            ...
            for (int j : g[num]) {
                ...
                if (v.marked) {
                    ...
                    if (v.color1 != color) {
                        if (!v.trySet(getNot(v.color1, color), g, vertex)) {
                            ok = false;
                        }
                    }
                }
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        if (v[i].get_new_color() == 0) {
            for (int color : {1, 2 ,3}) {
                for (Vertex& vertex : v) {
                    vertex.change_marked(false);
                }
                if (v[i].trySet(color, g, v)) {
                    break;
                }
            }
        }
