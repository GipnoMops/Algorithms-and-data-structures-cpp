#include <iostream>
#include <cmath>
#include <vector>
#include <memory.h>
#include <complex>

const int mod = 786433;
const size_t MAX_PRICE = 50005;
const int N = 100500;

typedef std::complex<long double> dou_cpx;

int cost[N];
int pref_sum[MAX_PRICE];

void fft(std::vector<dou_cpx> &a, bool invert) {
    int n = (int)a.size();
    if (n == 1) return;
    std::vector<dou_cpx> a0(n/2); std::vector<dou_cpx> a1(n/2);
    for (int i = 0, j = 0; i < n; i += 2, ++j) {
        a0[j] = a[i]; a1[j] = a[i + 1];
    }
    fft (a0, invert); fft (a1, invert);
    long double ang = 2 * M_PI / n * (invert ? -1 : 1);
    dou_cpx w (1),  wn (cos(ang), sin(ang));
    for (int i = 0; i < n / 2; ++i) {
        a[i] = a0[i] + w * a1[i];
        a[i+n / 2] = a0[i] - w * a1[i];
        if (invert) {
            a[i] /= 2;
            a[i+n/2] /= 2;
        }
        w *= wn;
    }
}

void multiply(const std::vector<int>& a, const std::vector<int>& b, std::vector<int>& res) {
    std::vector<dou_cpx> fa (a.begin(), a.end()),  fb (b.begin(), b.end());
    size_t n = 1;
    while (n < std::max(a.size(), b.size()))  n <<= 1;
    n <<= 1;
    fa.resize(n); fb.resize(n);
    fft(fa, false); fft(fb, false);
    for (size_t i = 0; i < n; ++i) fa[i] *= fb[i];
    fft(fa, true);
    size_t res_size = std::min(n, MAX_PRICE);
    res.resize(res_size);
    for (size_t i=0; i < res_size; ++i) {
        long long x = (long long)(fa[i].real() + 0.5);
        x %= (long long)mod;
        res[i] = (int)x;
    }
}

void binpow(std::vector<int>& a, int k, std::vector<int>& result) {
    while (k) {
        if (k & 1) {
            multiply(result, a, result);
        }
        multiply(a, a, a);
        k >>= 1;
    }
}


int main() {
    int n, k, q;
    std::cin >> n >> k >> q;
    int maxCost = 0;
    for (int i = 1; i <= n; ++i) {
        int x; std::cin >> x;
        cost[i] = x;
        maxCost = std::max(maxCost, cost[i]);
    }

    std::vector<int> ways(maxCost + 5, 0);
    for (int i = 1; i <= n; ++i) ++ways[cost[i]];

    std::vector<int> result = ways;

    binpow(ways, k - 1, result);

    while (result.size() < MAX_PRICE) result.push_back(0);

    pref_sum[0] = result[0];
    for (int i = 1; i < MAX_PRICE; ++i) {
        pref_sum[i] = (pref_sum[i - 1] + result[i]) % mod;
    }

    for (int i = 0; i < q; ++i) {
        int l, r;
        std::cin >> l >> r;
        int res = (pref_sum[r] - pref_sum[std::max(0, l - 1)] + mod) % mod;
        std::cout << res << "\n";
    }
    return 0;
}