...
void dfs (size_t now, std::vector<std::vector<std::pair<size_t, size_t>>>& g, size_t p,
    ...
    for (auto& [neig, order] : g[now]) {
        ...
        if (used[neig])
            ...
            dfs(neig, g, now, used, timer, tin, fup, ansver);
            fup[now] = std::min(fup[now], fup[neig]);
            if (fup[neig] > tin[now])
                ansver.push_back(order);
        }
    }
}
...
void find_bridges(std::vector<std::vector<std::pair<size_t, size_t>>>& g) {
    ...
    for (size_t i = 1; i < g.size(); ++i) {
        size_t timer = 0;
        if (!used[i])
            dfs(i, g, -1, used, timer, tin, fup, ansver);
    }

    std::sort(ansver.begin(), ansver.end());
    std::cout << ansver.size() << "\n";
...
int main() {
    ...
    for (size_t i = 0; i < e; ++i) {
        std::cin >> top1 >> top2;
       /* if (std::count(table[top1].begin(), table[top1].end(), std::make_pair(top2)) == 0 && top2 != top1) {
