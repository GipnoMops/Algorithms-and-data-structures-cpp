...
void dfs (size_t now, std::vector<std::vector<std::pair<size_t, size_t>>>& g, size_t p,
    ...
    for (auto& neig : g[now]) {
        if (neig.first == p)  continue;
        if (used[neig.first])
            fup[now] = std::min(fup[now], tin[neig.first]);
        else {
            dfs(neig.first, g, now, used, timer, tin, fup);
            fup[now] = std::min(fup[now], fup[neig.first]);
            /*if (fup[neig.first] > tin[now])
                std::cout << neig.second << "\n";*/
        }
    }
}
